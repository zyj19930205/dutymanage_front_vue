...
export default {
  ...
  mounted () {
    this.axios.get(this.serverUrl + '/getTempDutyResult', {
      params: {
        limit: 10,
        page: 1
      }
  ...
  methods: {
    ...
    selectDate (name) {
      this.axios.get(this.serverUrl + '/getDutyDateByEmpName/' + name)
        .then(response => {
    ...
    changeDutyOk () {
      ...
      this.axios.post(this.serverUrl + '/replaceEmpDutyResult', params)
        .then(response => {
          this.changedEmpDate = response.data
        })
        .catch(function (error) {
